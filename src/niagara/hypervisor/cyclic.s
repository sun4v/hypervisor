 ...
 */
	...
	.ident	"@(#)cyclic.s	1.4	05/08/20 SMI"
...
#include "util.h"
	...
	/* This is the core function cycluc_add() */
	stx	%g2, [%g6 + CPU_CY_HANDLER]	! save handler address
	stx	%g3, [%g6 + CPU_CY_ARG0]	! save handler args
	stx	%g4, [%g6 + CPU_CY_ARG1]	! save handler args

	/*
	 * Check if array full:
	 */
	ldx	[%g6 + CPU_CY_CB_LAST_TICK], %g2
	brnz,a	%g2, .cya_ret			! full: return error
	  mov	1, %g1				! status = 1

	ldx	[%g6 + CPU_ROOT], %g5		! ->config
	ldx	[%g6 + CPU_CY_CB_TICK], %g2	! first tick
	brnz,pt	%g2, .cya_1			! not empty: continue
	  rd	STICK, %g3			! get current time
	stx	%g3, [%g6 + CPU_CY_T0]		! empty: set t0
.cya_1:
	mov	1, %g2				! set INT_DIS (B63=1)
	sllx	%g2, 63, %g2			!     ..
	bset	%g2, %g4			!	..
	wrhpr	%g4, %hstick_cmpr		! inhibit the compare interrupt
...
.cya_3:
	/* g1=abs_tick */
	ldx	[%g6 + CPU_CY_T0], %g4		! normalize input to T0
	subcc	%g1, %g4, %g1			!     ..
	movlu	%xcc, %g0, %g1			! reverse - set to minimum
	/* g1=delta_tick */
	add	%g6, CPU_CY_CB, %g4		! ->cb[0]
...
.cya_7:
	...
	ldx	[%g6 + CPU_CY_HANDLER],%g2	! saved handler
	stx	%g2, [%g4 + CB_HANDLER]		!   store
	ldx	[%g6 + CPU_CY_ARG0], %g2	! saved arg0
	stx	%g2, [%g4 + CB_ARG0]		!   store
	ldx	[%g6 + CPU_CY_ARG1], %g2	! saved arg1
	stx	%g2, [%g4 + CB_ARG1]		!   store
	/*
	 * Setup new timer interrupt:
	 */
	ldx	[%g6 + CPU_CY_T0], %g1		! T0 (abs base for cyclic[].tick)
	ldx	[%g6 + CPU_CY_CB_TICK], %g2	! Td (next delta time)
...
.cya_ret:
	...
	mov	HSCRATCH0, %g4
	ldxa	[%g4]ASI_HSCRATCHPAD, %g4

	add	%g4, CPU_CY_CB, %g4			! ->cb[0]
...
.cyr_9:
	...
	CPU_STRUCT(%g6)					! ->cpu
	add	%g6, CPU_CY_CB, %g5			! ->cb[0]

	ldx	[%g5 + CB_HANDLER], %g4			! cb[0].handler
	brz,a,pn %g4, .cyp_9				! null: return g4=null
	  clrx	[%g6 + CPU_CY_T0]			!       reset time basis
	/*
	 * Update time basis:
	 */
	ldx	[%g6 + CPU_CY_T0], %g3			! T0
...
.cyp_9:
	...
	brgez	%g1, .hsi_8			!   yes: clear int & return
	  nop
	CPU_STRUCT(%g6)				! ->cpu
	ldx	[%g6 + CPU_CY_CB_HANDLER], %g2	! first handler
	brz,a	%g2, .hsi_8			! empty: clear int & return
	  nop
	ldx	[%g6 + CPU_CY_CB_TICK], %g2	! Td (next delta time)
	ldx	[%g6 + CPU_CY_T0], %g4		! T0 (abs base for cyclic[].tick)
...
.hsi_8:
	 ...
	 */
	rdhpr	%hintp, %g1			! clear only our bit
	bclr	HSTICKPEND_HSP, %g1		! (future compatability)
	wrhpr	%g1, %hintp
